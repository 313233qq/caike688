...
var firstTopDisLayer = cc.Layer.extend({
	...
	firstBtmDisLayer = cc.Layer.extend({
		...
		setCurData: function(a) {
			...
			if (b % 2 ===0) {
				...
				this._curDanShuang.setString("和");
			} else {
    ...
    g_hostIp = "http://"+document.domain+"/Home/Get/getEr75sc?t="+ Math.random(),
	...
	pk10 = cc.Layer.extend({
		...
		timeLableRunning: function() {
			if (0 < this._timeNumber) {
				...
				5 == this._timeNumber && (g_isMusicPlay ? cc.AudioEngine.getInstance().rewindMusic() : (cc.AudioEngine.getInstance().playMusic(g_music, !0),
				 g_isMusicPlay = !0));
			}
			else {
