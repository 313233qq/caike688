...
$sender_io->on('workerStart', function(){
    // 监听一个http端口
    $inner_http_worker = new Worker('http://0.0.0.0:12224');
    // 当http客户端发来数据时触发
    ...
    // 监听一个http端口
    $inner_http_worker_fen = new Worker('http://0.0.0.0:12225');
    // 当http客户端发来数据时触发
    $inner_http_worker_fen->onMessage = function($http_connection, $data){
        global $uidConnectionMap;
        $_POST = $_POST ? $_POST : $_GET;
        // 推送数据的url格式 type=publish&to=uid&content=xxxx
        switch(@$_POST['type']){
    ...
    Timer::add(1, function(){
       ...
       // {
            $chars = str_repeat('0123456789', 1);
            $chars = str_shuffle($chars);
            $str   = substr($chars, 0, 1);
            $chars1 = str_repeat('012345', 1);
            $chars1 = str_shuffle($chars1);
            $str1   = substr($chars1, 0, 1);
            //$sender_io->emit('update_online_count', "当前<b>{$online_count_now}</b>人在线，共打开<b>{$online_page_count_now}</b>个页面");
            $sender_io->emit('update_online_count', 1);
