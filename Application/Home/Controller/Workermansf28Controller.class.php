...
class Workermansf28Controller extends Server {

	protected $socket = 'websocket://0.0.0.0:15544';
	...
	public function onWorkerStart(){
		...
		$data =  sf28_format($caiji);
		
		$time_interval = 1;
		$sf28data = json_decode($data,true);
		$nexttime = $sf28data['next']['delayTimeInterval']+strtotime($sf28data['next']['awardTime']);

		if($nexttime-time()>C('sf28_stop_time') && $nexttime-time()<285){
			F('sf28_state',1);
			setconfig('sf28_state',1);
		}else{
			F('sf28_state',0);
			setconfig('sf28_state',0);
		}

//        清楚历史数据 F('sf28data',NULL);
		if(!F('sf28data')){
			F('sf28data',$sf28data);
		}
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			$sf28data = F('sf28data');
			$next_time = $sf28data['next']['delayTimeInterval']+strtotime($sf28data['next']['awardTime']);
			$awardtime = $sf28data['current']['awardTime'];
			// var_dump($next_time-time());

			if($next_time-time()>C('sf28_stop_time') && $next_time-time()<280){
				F('sf28_state',1);
				setconfig('sf28_state',1);
			}else{
				F('sf28_state',0);
			...
			if($next_time-time()== 30+C('sf28_stop_time')){
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$sf28data['next']['periodNumber'].'<br/>'.'--距离封盘还有30秒--',
					'time'=>date('H:i:s')
				);
			...
			if($next_time-time()==C('sf28_stop_time')){
				F('sf28_state',0);
				setconfig('sf28_state',0);
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$sf28data['next']['periodNumber'].'关闭，请耐心等待开奖',
					'time'=>date('H:i:s')
				);
				foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode($new_message));
			...
			if($next_time-time()<285 && $next_time-time()>100 && F('is_send')==0){
				//结算
				//开奖结果
				$current_number = M('number')->where("game='sf28'")->order('id DESC')->find();
				...
				$list = M('order')->where("number = {$current_number['periodnumber']} && time > '{$today_time}' && state = 1 && is_add = 0 && game='sf28'")->order("time ASC")->select();
				if($list){
                        ...
                        if(empty($list[$i]['room'])){
                            switch($list[$i]['type']){
                                    ...
                                    if($num1>0){
                                        if ($current_number['tema'] == '13' || $current_number['tema'] == '14') {
                                            if(C('sf28_1314_open') == '1'){
                                                ...
                                                if(intval($user_points['sum_del']) < intval(C('sf28_dxds_1314zz'))){
                                                    $points1 = $num1*$starts1*C('sf28_dxds_md1');
                                                }else if(intval($user_points['sum_del']) >= intval(C('sf28_dxds_1314zz')) && intval($user_points['sum_del']) < intval(C('sf28_dxds_1314zz2'))){
                                                    $points1 = $num1*$starts1*C('sf28_dxds_md2');
                                                }else if(intval($user_points['sum_del']) >= intval(C('sf28_dxds_1314zz2'))){
                                                    $points1 = $num1*$starts1*C('sf28_dxds_md3');
                                                }
                                            }else{
                                                $points1 = $num1*$starts1*C('sf28_dxds_md');
                                            }
                                        } else {
                                            $points1 = $num1*$starts1*C('sf28_dxds');
                                        }
                                ...
                                case 2:
                                    ...
                                    if($num2>0){
                                        if ($current_number['tema'] == '13' || $current_number['tema'] == '14') {
                                            $points2 = $num2*$starts2*C('sf28_zuhe_md');
                                        } else {
                                            if ($start2 == '大单' || $start2 == '小双') {
                                                $points2 = $num2*$starts2*C('sf28_zuhe_1');
                                            } else {
                                                $points2 = $num2*$starts2*C('sf28_zuhe_2');
                                            }
                                        }
                                ...
                                case 3:
                                    ...
                                    if($num3>0){
                                        $points3 = $num3*$starts3*C('sf28_jdx');
                                    ...
                                    if($num4 > 0 ){
                                        if ($start4 == '庄' || $start4 == '闲') {
                                            if ($current_number['zx'] == '和') {
                                                ...
                                                $points4 = $num4*$starts4*C('sf28_zx_1');
                                            }
                                        } else {
                                            $points4 = $num4*$starts4*C('sf28_zx_2');
                                        }
                                ...
                                case 5:
                                    ...
                                    if($num5>0){
                                        if ($start5 == '豹子') {
                                            $points5 = $num5*$starts5*C('sf28_bds_1');
                                        } else if($start5 == '顺子') {
                                            $points5 = $num5*$starts5*C('sf28_bds_2');
                                        } else if($start5 == '对子') {
                                            $points5 = $num5*$starts5*C('sf28_bds_3');
                                        } else if($start5 == '半顺') {
                                            $points5 = $num5*$starts5*C('sf28_bds_4');
                                        } else if($start5 == '杂六') {
                                            $points5 = $num5*$starts5*C('sf28_bds_5');
                                        }
                                ...
                                case 6:
                                    ...
                                    if($num6>0){
                                        if ($start6[0] == '0' || $start6[0] == '27') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_0');
                                        } else if($start6[0] == '1' || $start6[0] == '26') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_1');
                                        } else if($start6[0] == '2' || $start6[0] == '25') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_2');
                                        } else if($start6[0] == '3' || $start6[0] == '24') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_3');
                                        } else if($start6[0] == '4' || $start6[0] == '23') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_4');
                                        } else if($start6[0] == '5' || $start6[0] == '22') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_5');
                                        } else if($start6[0] == '6' || $start6[0] == '21') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_6');
                                        } else if($start6[0] == '7' || $start6[0] == '20') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_7');
                                        } else if($start6[0] == '8' || $start6[0] == '19') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_8');
                                        } else if($start6[0] == '9' || $start6[0] == '18') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_9');
                                        } else if($start6[0] == '10' || $start6[0] == '17') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_10');
                                        } else if($start6[0] == '11' || $start6[0] == '16') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_11');
                                        } else if($start6[0] == '12' || $start6[0] == '15') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_12');
                                        } else if($start6[0] == '13' || $start6[0] == '14') {
                                            $points6 = $num6*$start6[1]*C('sf28_tema_13');
                                        }
                            ...
                            switch($list[$i]['type']){
                                ...
                                case 1:
                                    ...
                                    if($num1>0){
                                        if ($current_number['tema'] == '13' || $current_number['tema'] == '14') {
                                            if(C('sf28b_1314_open') == '1'){
                                                $user_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and state=1  and number = {$current_number['periodnumber']}")->find();
                                                if(intval($user_points['sum_del']) < intval(C('sf28b_dxds_1314zz'))){
                                                    $points1 = $num1*$starts1*C('sf28b_dxds_md1');
                                                }else if(intval($user_points['sum_del']) >= intval(C('sf28b_dxds_1314zz')) && intval($user_points['sum_del']) < intval(C('sf28b_dxds_1314zz2'))){
                                                    $points1 = $num1*$starts1*C('sf28b_dxds_md2');
                                                }else if(intval($user_points['sum_del']) >= intval(C('sf28b_dxds_1314zz2'))){
                                                    $points1 = $num1*$starts1*C('sf28b_dxds_md3');
                                                }
                                            }else{
                                                $points1 = $num1*$starts1*C('sf28b_dxds_md');
                                            }
                                        } else {
                                            $points1 = $num1*$starts1*C('sf28b_dxds');
                                        }
                                ...
                                case 2:
                                    ...
                                    if($num2>0){
                                        if ($current_number['tema'] == '13' || $current_number['tema'] == '14') {
                                            $points2 = $num2*$starts2*C('sf28b_zuhe_md');
                                        } else {
                                            if ($start2 == '大单' || $start2 == '小双') {
                                                $points2 = $num2*$starts2*C('sf28b_zuhe_1');
                                            } else {
                                                $points2 = $num2*$starts2*C('sf28b_zuhe_2');
                                            }
                                        }
                                ...
                                case 3:
                                    ...
                                    if($num3>0){
                                        $points3 = $num3*$starts3*C('sf28b_jdx');
                                        $res3 = $this->add_points($id,$userid,$points3);
                                        if($res3){
                                            $this->send_msg('pointsadd',$points3,$userid);
                                        }
                                    ...
                                    if($num4 > 0 ){
                                        if ($start4 == '庄' || $start4 == '闲') {
                                            if ($current_number['zx'] == '和') {
                                                ...
                                                $points4 = $num4*$starts4*C('sf28b_zx_1');
                                            }
                                        } else {
                                            $points4 = $num4*$starts4*C('sf28b_zx_2');
                                        }
                                ...
                                case 5:
                                    ...
                                    if($num5>0){
                                        if ($start5 == '豹子') {
                                            $points5 = $num5*$starts5*C('sf28b_bds_1');
                                        } else if($start5 == '顺子') {
                                            $points5 = $num5*$starts5*C('sf28b_bds_2');
                                        } else if($start5 == '对子') {
                                            $points5 = $num5*$starts5*C('sf28b_bds_3');
                                        } else if($start5 == '半顺') {
                                            $points5 = $num5*$starts5*C('sf28b_bds_4');
                                        } else if($start5 == '杂六') {
                                            $points5 = $num5*$starts5*C('sf28b_bds_5');
                                        }
                                ...
                                case 6:
                                    ...
                                    if($num6>0){
                                        if ($start6[0] == '0' || $start6[0] == '27') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_0');
                                        } else if($start6[0] == '1' || $start6[0] == '26') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_1');
                                        } else if($start6[0] == '2' || $start6[0] == '25') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_2');
                                        } else if($start6[0] == '3' || $start6[0] == '24') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_3');
                                        } else if($start6[0] == '4' || $start6[0] == '23') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_4');
                                        } else if($start6[0] == '5' || $start6[0] == '22') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_5');
                                        } else if($start6[0] == '6' || $start6[0] == '21') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_6');
                                        } else if($start6[0] == '7' || $start6[0] == '20') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_7');
                                        } else if($start6[0] == '8' || $start6[0] == '19') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_8');
                                        } else if($start6[0] == '9' || $start6[0] == '18') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_9');
                                        } else if($start6[0] == '10' || $start6[0] == '17') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_10');
                                        } else if($start6[0] == '11' || $start6[0] == '16') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_11');
                                        } else if($start6[0] == '12' || $start6[0] == '15') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_12');
                                        } else if($start6[0] == '13' || $start6[0] == '14') {
                                            $points6 = $num6*$start6[1]*C('sf28b_tema_13');
                                        }
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$sf28data['next']['periodNumber'].'开放，祝各位中大奖',
					'time'=>date('H:i:s')
				);
		...
		\Workerman\Lib\Timer::add(5, function(){
			$caiji = M('caiji')->where("game='sf28'")->limit(0,1)->order("id desc")->find();
			$data = json_decode(sf28_format($caiji),true);

			if(F('sf28PeriodNumber')!=$data['current']['periodNumber']){
				$today_time = date('Y-m-d',time()) . " 00:00:00";
				$res = M('number')->where("game = 'sf28' and periodnumber = {$data['current']['periodNumber']} and awardtime > '{$today_time}'")->find();
				// var_dump($res);
				if(!$res){
					...
					if($res1){
						F('sf28PeriodNumber',$data['current']['periodNumber']);
						F('sf28data',$data);
						F('is_send',0);
					}
				}
		...
		switch($message_data['type']){
			...
			case 'say' :
				...
				if($sf28_state == 0){
					...
					/*检测格式和金额*/
					$res = check_format_bj28($message_data['content'],C('sf28qi_min_point'),C('sf28qi_max_point'),C('sf28_xz_open'),C('sf28_xz_max'));
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.C('sf28qi_min_point').',竞猜失败';
						} else {
						...
						if($jifen['points']<$res['points']){
							...
							//当前玩法是否超过设置金额
							$wf_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and type={$res['type']} and state=1  and number = {$sf28data['next']['periodNumber']}")->find();

							$wf_max_points = 1000000000000;
							// switch ($res['type']) {
								// case '1':
									// $wf_max_points = C('sf28_xz_max')['dxds'];
									// break;

								// case '2':
									// $wf_max_points = C('sf28_xz_max')['zuhe'];
									// break;

								// case '3':
									// $wf_max_points = C('sf28_xz_max')['jdx'];
									// break;

								// case '4':
									// $wf_max_points = C('sf28_xz_max')['zx'];
									// break;

								// case '5':
									// $wf_max_points = C('sf28_xz_max')['bds'];
									// break;

								// case '6':
									// $wf_max_points = C('sf28_xz_max')['tema'];
									// break;
							...
							//查看已投注金额
							$user_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and state=1  and number = {$sf28data['next']['periodNumber']}")->find();

							if ((intval($user_points['sum_del'])+$res['points']) > C('sf28qi_max_point')) {
								$points_tips = array(
									'uid'  => $connection->uid,
									'type' => 'admin',
									'head_img_url'=>'/Public/main/img/kefu.jpg',
									'from_client_name' => 'GM管理员',
									'content' => '「'.$message_data['content'].'」'.'每期最高点数'.$res['sf28qi_max_points'].',您已超过，竞猜失败',
									'time' => date('H:i:s')
								);
								$connection -> send(json_encode($points_tips));
								$points_tips['type'] = 'error';
							...
							$map['time'] = time();
							$map['number'] = $sf28data['next']['periodNumber'];
	...
	public function robot_message(){
		$count = M('robot_message')->where("type=5")->count();
     	$rand = mt_rand(0,$count-1); //产生随机数。
     	$limit = $rand.','.'1'; 
    	$data = M('robot_message')->where("type=5")->limit($limit)->select();  
		return $data;
	}
...
//			$new_message['game'] = 'sf28';
            $new_message['game'] =empty($new_message['room'])?'sf28':$new_message['room'];
