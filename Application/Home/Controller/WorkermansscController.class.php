...
class WorkermansscController extends Server {
	...
	public function onWorkerStart(){
		...
		\Workerman\Lib\Timer::add(5, function(){
			...
			if(F('sscPeriodNumber')!=$data['current']['periodNumber']){
				$today_time = date('Y-m-d',time()) . " 00:00:00";
				$res = M('number')->where("game = 'ssc' and periodnumber = '{$data['current']['periodNumber']}' and awardtime > '{$today_time}'")->find();
				if(!$res){
			...
			case 'say' :
				...
				if($ssc_state == 0){
					...
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.C('sscqi_min_point').',竞猜失败';
						} else {
						...
						if($jifen['points']<$res['points']){
							...
							// switch ($res['type']) {
								...
								// case '7':
									...
									// break;
								// case '8':
									// $wf_max_points = C('ssc_xz_max')['lhh_he'];
									// break;
