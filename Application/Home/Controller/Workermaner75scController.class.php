...
class Workermaner75scController extends Server {
	...
	public function onWorkerStart(){
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			if($next_time-time()== 24+C('er75sc_stop_time')){
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$er75scdata['next']['periodNumber'].'<br/>'.'--距离封盘还有30秒--',
					'time'=>date('H:i:s')
				);
			...
			if($next_time-time()==C('er75sc_stop_time')-6){
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$er75scdata['next']['periodNumber'].'关闭，请耐心等待开奖',
					'time'=>date('H:i:s')
				);
				foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode($new_message));
			...
			if((($next_time-time()>=58) || (time() > strtotime("23:54:00") && time() < strtotime("23:59:00"))) && F('is_send')==0){
				...
				//结算
				//开奖结果
				$current_number = M('number')->where("game='er75sc'")->order('id DESC')->find();
				...
				$list = M('order')->where("number = {$current_number['periodnumber']} && time > '{$today_time}' && state = 1 && is_add = 0 && game='er75sc'")->order("time ASC")->select();
					...
					for($i=0;$i<count($list);$i++){
						...
						switch($list[$i]['type']){
							...
							case 1:
								...
								if($num1>0){
									$points1 = $num1*$start1[1]*C('er75sc_dxds');
							...
							case 2:
								...
								if($num2>0){
									$points2 = $num2*$ya*C('er75sc_chehao');
							...
							case 3:
								...
								if($num3>0){
									if($start3[1]=='大单' || $start3[1]=='小双'){
										$points3 = $num3*$start3[2]*C('er75sc_zuhe_1');
									}else{
										$points3 = $num3*$start3[2]*C('er75sc_zuhe_2');
									}
							...
							case 4:
								...
								if($num4>0){
									$points4 = $num4*$start4[2]*C('er75sc_lh');
							...
							case 6:
								...
								if($num6>0){
									$points6 = $num6*$start6[2]*C('er75sc_gy');
							...
							case 7:
								...
								$num7 = 0;
								if($current_number['tema'] == 11 && C('er75sc_gy_set') == 2){
									$num7 = 2;
									...
									if ($num7 == 1) {
										if (C('er75sc_gy_set') == 1) {
											if($start7=='大' || $start7=='双'){
												$points7 = $starts7*C('er75sc_tema_1');
											}else{
												$points7 = $starts7*C('er75sc_tema_2');
											}
										} else{
											$points7 = $starts7*C('er75sc_tema');
										}
									...
									for($a=0;$a<count($starts9);$a++){
										if($current_number['tema_dw']==$starts9[$a]){
											if($starts9[$a]=='A' || $starts9[$a]=='C'){
												$points9 = $start9[1]*C('er75sc_tema_qd_1');
											}else{
												$points9 = $start9[1]*C('er75sc_tema_qd_2');
											}
											$num9 = 1;
										}
									}
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$er75scdata['next']['periodNumber'].'开放，祝各位中大奖',
					'time'=>date('H:i:s')
				);
		...
		\Workerman\Lib\Timer::add(5, function(){
			$caiji = M('caiji')->where("game='er75sc'")->limit(0,1)->order("id desc")->find();
			$data = json_decode(er75sc_format($caiji),true);
			
			if(F('er75scPeriodNumber')!=$data['current']['periodNumber']){
				$today_time = date('Y-m-d',time()) . " 00:00:00";
				$res = M('number')->where("game = 'er75sc' and periodnumber = {$data['current']['periodNumber']} and awardtime > '{$today_time}'")->find();
				if(!$res){
		...
		switch($message_data['type']){
			...
			case 'say' :
				...
				if($er75sc_state == 0){
					...
					/*检测格式和金额*/
					$res = check_format($message_data['content'],C('er75scqi_min_point'),C('er75scqi_max_point'),C('er75sc_xz_open'),C('er75sc_xz_max'));
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.C('er75scqi_min_point').',竞猜失败';
						} else {
						...
						if($jifen['points']<$res['points']){
							...
							if ((intval($user_points['sum_del'])+$res['points']) > C('er75scqi_max_point')) {
								$points_tips = array(
									...
									'head_img_url'=>'/Public/main/img/kefu.jpg',
									'from_client_name' => 'GM管理员',
									'content' => '「'.$message_data['content'].'」'.'每期最高点数'.C('er75scqi_max_point').',您已超过，竞猜失败',
									'time' => date('H:i:s')
								);
								$connection -> send(json_encode($points_tips));
								$points_tips['type'] = 'error';
							...
							$map['time'] = time();
							$map['number'] = $er75scdata['next']['periodNumber'];
