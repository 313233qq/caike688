...
class Workermanpk10Controller extends Server {
	...
	public function onWorkerStart(){
		...
		if($nexttime-time()>C('pk10_stop_time')-6 && $nexttime-time()<280 && time()>$beginToday && time()<$endToday){
			F('pk10_state',1);
		}else{
			F('pk10_state',0);
		}
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			if($next_time-time()>C('pk10_stop_time')-6 && $next_time-time()<280 && time()>$beginToday && time()<$endToday ){
				F('pk10_state',1);
			}else{
				F('pk10_state',0);
			}
			...
			if((($next_time-time()<255 && $next_time-time()>90) || (time() > strtotime("23:54:00") && time() < strtotime("23:59:00"))) && F('is_send')==0){
				//结算
				//开奖结果
			...
			case 'say' :
				...
				if($pk10_state == 0){
					...
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.C('pk10_min_point').',竞猜失败';
						} else {
