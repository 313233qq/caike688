...
class Workermanxjp28Controller extends Server {
	...
	public function onWorkerStart(){
		...
		$caiji = M('caiji')->where("game='xjp28'")->limit(0,1)->order("id desc")->find();
		$data =  xjp28_format($caiji);
		
		$time_interval = 1;
		$xjp28data = json_decode($data,true);
		$nexttime = $xjp28data['next']['delayTimeInterval']+strtotime($xjp28data['next']['awardTime']);
		if($nexttime-time()>C('xjp28_stop_time') && $nexttime-time()<300 && time()>$beginToday && time()<$endToday){
			F('xjp28_state',1);
			setconfig('xjp28_state',1);
		}else{
			F('xjp28_state',0);
			setconfig('xjp28_state',0);
		}
		if(!F('xjp28data')){
			F('xjp28data',$xjp28data);
		}
		if(!F('is_send')){
			F('is_send',1);
		}
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			$xjp28data = F('xjp28data');
			$next_time = $xjp28data['next']['delayTimeInterval']+strtotime($xjp28data['next']['awardTime']);
			$awardtime = $xjp28data['current']['awardTime'];
			// var_dump($next_time-time());
			if($next_time-time()>C('xjp28_stop_time') && $next_time-time()<300 && time()>$beginToday && time()<$endToday ){
				F('xjp28_state',1);
				setconfig('xjp28_state',1);
			}else{
				F('xjp28_state',0);
			...
			if($next_time-time()== 30+C('xjp28_stop_time')){
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$xjp28data['next']['periodNumber'].'<br/>'.'--距离封盘还有30秒--',
					'time'=>date('H:i:s')
				);
			...
			if($next_time-time()==C('xjp28_stop_time')){
				F('xjp28_state',0);
				setconfig('xjp28_state',0);
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$xjp28data['next']['periodNumber'].'关闭，请耐心等待开奖',
					'time'=>date('H:i:s')
				);
				foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode($new_message));
			...
			if((($next_time-time()<300 && $next_time-time()>100) || (time() > strtotime("23:52:00") && time() < strtotime("23:55:00"))) && F('is_send')==0){
				//结算
				//开奖结果
				$current_number = M('number')->where("game='xjp28'")->order('id DESC')->find();
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$xjp28data['next']['periodNumber'].'开放，祝各位中大奖',
					'time'=>date('H:i:s')
				);
		...
		\Workerman\Lib\Timer::add(5, function(){
			$caiji = M('caiji')->where("game='xjp28'")->limit(0,1)->order("id desc")->find();
			$data = json_decode(xjp28_format($caiji),true);

			if(F('xjp28PeriodNumber')!=$data['current']['periodNumber']){
				$today_time = date('Y-m-d',time()) . " 00:00:00";
				$res = M('number')->where("game = 'xjp28' and periodnumber = {$data['current']['periodNumber']} and awardtime > '{$today_time}'")->find();
				// var_dump($res);
				if(!$res){
					...
					if($res1){
						F('xjp28PeriodNumber',$data['current']['periodNumber']);
						F('xjp28data',$data);
						F('is_send',0);
					}
				}
		...
		switch($message_data['type']){
			...
			case 'say' :
				...
				if($xjp28_state == 0){
					...
					/*检测格式和金额*/
					$res = check_format_bj28($message_data['content'],C('xjp28qi_min_point'),C('xjp28qi_max_point'),C('xjp28_xz_open'),C('xjp28_xz_max'));
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.C('xjp28qi_min_point').',竞猜失败';
						} else {
						...
						if($jifen['points']<$res['points']){
							...
							//当前玩法是否超过设置金额
							$wf_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and type={$res['type']} and state=1  and number = {$xjp28data['next']['periodNumber']}")->find();

							$wf_max_points = 0;
							switch ($res['type']) {
								case '1':
									$wf_max_points = C('xjp28_xz_max')['dxds'];
									break;

								case '2':
									$wf_max_points = C('xjp28_xz_max')['zuhe'];
									break;

								case '3':
									$wf_max_points = C('xjp28_xz_max')['jdx'];
									break;

								case '4':
									$wf_max_points = C('xjp28_xz_max')['zx'];
									break;

								case '5':
									$wf_max_points = C('xjp28_xz_max')['bds'];
									break;

								case '6':
									$wf_max_points = C('xjp28_xz_max')['tema'];
									break;
							...
							//查看已投注金额
							$user_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and state=1  and number = {$xjp28data['next']['periodNumber']}")->find();

							if ((intval($user_points['sum_del'])+$res['points']) > C('xjp28qi_max_point')) {
								$points_tips = array(
									'uid'  => $connection->uid,
									'type' => 'admin',
									'head_img_url'=>'/Public/main/img/kefu.jpg',
									'from_client_name' => 'GM管理员',
									'content' => '「'.$message_data['content'].'」'.'每期最高点数'.$res['xjp28qi_max_points'].',您已超过，竞猜失败',
									'time' => date('H:i:s')
								);
								$connection -> send(json_encode($points_tips));
								$points_tips['type'] = 'error';
							...
							$map['time'] = time();
							$map['number'] = $xjp28data['next']['periodNumber'];
