...
class WorkermansfcController extends Server {

	protected $socket = 'websocket://0.0.0.0:15543';
	...
	public function onWorkerStart(){
		...
		if($nexttime-time()>C('sfc_stop_time') && $nexttime-time()<$st_time){
			F('sfc_state',1);
			setconfig('sfc_state',1);
		}else{
			F('sfc_state',0);
			setconfig('sfc_state',0);
		}
		if(!F('sfcdata')){
			F('sfcdata',$scdata);
		}
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			$sfcdata = F('sfcdata');
			$next_time = $sfcdata['next']['delayTimeInterval']+strtotime($sfcdata['next']['awardTime']);
			$awardtime = $sfcdata['current']['awardTime'];

			$time = time();
			$st_time = 180;

			if($next_time-time()>C('sfc_stop_time') && $next_time-time()<$st_time ){
				F('sfc_state',1);
				setconfig('sfc_state',1);
			}else{
				F('sfc_state',0);
			...
			if($next_time-time()==30+C('sfc_stop_time')){
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$sfcdata['next']['periodNumber'].'<br/>'.'--距离封盘还有30秒--',
					'time'=>date('H:i:s')
				);
			...
			if($next_time-time()==C('sfc_stop_time')){
				F('sfc_state',0);
				setconfig('sfc_state',0);
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$sfcdata['next']['periodNumber'].'关闭，请耐心等待开奖',
					'time'=>date('H:i:s')
				);
				foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode($new_message));
			...
			if(($next_time-time()<$st_time && $next_time-time()>$st_time-150)  && F('is_send')==0){
				//结算
				//开奖结果
				$current_number = M('number')->where("game = 'sfc'")->order('id DESC')->find();
				...
				$list = M('order')->where("number = {$current_number['periodnumber']} && state = 1 && is_add = 0 && game = 'sfc'")->order("time ASC")->select();
					...
					for($i=0;$i<count($list);$i++){
						...
						switch($list[$i]['type']){
							...
							case 2:
								...
								if($num2>0){
									if ($start2 == '龙' || $start2 == '虎') {
										$points2 = $num2*$starts2*C('sfc_lhh_1');
									} else if($start2 == '合') {
										$points2 = $num2*$starts2*C('sfc_lhh_2');
									}
							...
							case 3:
								...
								if($num3>0){
									$points3 = $num3*$start3[2]*C('sfc_dxds');
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$sfcdata['next']['periodNumber'].'开放，祝各位中大奖',
					'time'=>date('H:i:s')
				);
		...
		\Workerman\Lib\Timer::add(5, function(){
			$caiji = M('caiji')->where("game='sfc'")->limit(0,1)->order("id desc")->find();
			$data = json_decode(sfc_format($caiji),true);
			if(F('sfcPeriodNumber')!=$data['current']['periodNumber']){
				$today_time = date('Y-m-d',time()) . " 00:00:00";
				$res = M('number')->where("game = 'sfc' and periodnumber = '{$data['current']['periodNumber']}' and awardtime > '{$today_time}'")->find();
				if(!$res){
					...
					if($res1){
						F('sfcPeriodNumber',$data['current']['periodNumber']);
						F('sfcdata',$data);
						F('is_send',0);
		...
		switch($message_data['type']){
			...
			case 'say' :
				...
				if($sfc_state == 0){
					...
					$time = time();
					/*if (F('game') == 'sfc' && $time > strtotime("02:00:00") &&$time < strtotime("10:00:00")) {
						$content_msg =  "三分彩未开盘，请等待开盘";
						$new_message = array(
							'uid'  => $connection->uid,
							'type' => 'admin',
							'head_img_url'=>'/Public/main/img/kefu.jpg',
					...
					/*检测格式和金额*/
					$res = check_format_ssc($message_data['content'],C('sfcqi_min_point'),C('sfcqi_max_point'),C('sfc_xz_open'),C('sfc_xz_max'));
					if($res['error']==0 || $res['error']==2){
						...
						if ($res['error'] == 0) {
							$content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.C('sfcqi_min_point').',竞猜失败';
						} else {
						...
						if($jifen['points']<$res['points']){
							...
							//当前玩法是否超过设置金额
							$wf_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and type={$res['type']} and state=1  and number = {$sfcdata['next']['periodNumber']}")->find();

							$wf_max_points = 1000000000000;
							// switch ($res['type']) {
								// case '1':
									// $wf_max_points = C('sfc_xz_max')['dwq'];
									// break;

								// case '2':
									// $wf_max_points = C('sfc_xz_max')['lhh'];
									// break;

								// case '3':
									// $wf_max_points = C('sfc_xz_max')['dxds'];
									// break;

								// case '4':
									// $wf_max_points = C('sfc_xz_max')['zdxds'];
									// break;

								// case '5':
									// $wf_max_points = C('sfc_xz_max')['qzh'];
									// break;

								// case '6':
									// $wf_max_points = C('sfc_xz_max')['tema_qd'];
									// break;
								// case '7':
									// $wf_max_points = C('sfc_xz_max')['he'];
									// break;
								// case '8':
									// $wf_max_points = C('sfc_xz_max')['lhh_he'];
									// break;
							...
							//车号限制  123/23/100
							if ($res['type'] == '1') {
								$type_list = M('order')->where("userid = {$userid} and state=1  and number = {$sfcdata['next']['periodNumber']} and type = 1")->select();
								$type_data = array();
							...
							//查看已投注金额
							$user_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and state=1  and number = {$sfcdata['next']['periodNumber']}")->find();

							if ((intval($user_points['sum_del'])+$res['points']) > C('sfcqi_max_point')) {
								$points_tips = array(
									'uid'  => $connection->uid,
									'type' => 'admin',
									'head_img_url'=>'/Public/main/img/kefu.jpg',
									'from_client_name' => 'GM管理员',
									'content' => '「'.$message_data['content'].'」'.'每期最高点数'.$res['sfcqi_max_points'].',您已超过，竞猜失败',
									'time' => date('H:i:s')
								);
								$connection -> send(json_encode($points_tips));
								$points_tips['type'] = 'error';
							...
							$map['time'] = time();
							$map['number'] = $sfcdata['next']['periodNumber'];
	...
	public function robot_message(){
		$count = M('robot_message')->where("type=4")->count();
     	$rand = mt_rand(0,$count-1); //产生随机数。
     	$limit = $rand.','.'1'; 
    	$data = M('robot_message')->where("type=4")->limit($limit)->select();  
		return $data;
	}
